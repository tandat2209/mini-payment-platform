...
import { type FundingWebhookStore } from '../domain/funding.repositories';
...
@Injectable()
export class SqlFundingWebhookStore implements FundingWebhookStore {
  ...
  async markProcessingStatus(
    ...
    if (!event) {
      throw new Error('Webhook event could not be loaded after status update');
    }
