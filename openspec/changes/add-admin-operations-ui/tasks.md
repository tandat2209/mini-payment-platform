...
- [ ] 1.1 Add admin API routes and DTOs for triggering simulator funding requests through `apps/api`.
- [ ] 1.2 Add admin API routes and query/repository support for listing platform-wide user transactions with filters and pagination.
- [ ] 1.3 Add admin API routes and query/repository support for transaction detail, ledger transaction list, and ledger entry detail views.
...
- [ ] 2.2 Build the admin simulator form and result panel that calls the admin simulator endpoint with provider-style funding inputs.
...
- [ ] 3.1 Connect the admin web pages to the new admin APIs with loading, empty, and error states.
