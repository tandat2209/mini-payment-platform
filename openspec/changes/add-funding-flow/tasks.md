...
- [ ] 2.1 Add a demo provider webhook endpoint and request contract that records raw funding webhook deliveries in `webhook_events`.
...
- [ ] 3.1 Implement the simulator funding trigger endpoint and environment-driven outbound delivery to the API webhook endpoint.
- [ ] 3.2 Support explicit or generated provider event identifiers in the simulator response so duplicate-delivery testing is possible.
...
- [ ] 4.3 Document the local demo flow for opening `Add money`, triggering simulator funding, and checking the resulting account updates.
